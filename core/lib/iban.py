...
"""iban.py 1.5a - Create or check International Bank Account Numbers (IBAN).

Copyright (C) 2002-2010, Thomas Günther <tom@toms-cafe.de>
Modified: 2014, Jan Holthuis <holthuis.jan@googlemail.com>
...
class Country:
    ...
    def bank_lng(self):
        total = 0
        for part in self.bank:
                total += part[0]
        return total

    def acc_lng(self):
        total = 0
        for part in self.acc:
                total += part[0]
        return total
...
def print_iban_parts(iban):
    ...
    try:
        ...
        print( err )
        return ()
    print( "  Correct IBAN: %s >> %s %s %s %s" % (iban, code, checksum, bank, account) )
    return code, checksum, bank, account
